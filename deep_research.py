...
mcp = FastMCP("deep_reserch")

if not os.environ.get("OPENAI_API_KEY"):
    logger.error("WARN: OPENAI_API_KEY環境変数が設定されていません")
elif not os.environ.get("HF_TOKEN"):
    logger.error("WARN: HF_TOKEN環境変数が設定されていません")
elif not os.environ.get("SERPER_API_KEY"):
    logger.error("WARN: SERPER_API_KEY環境変数が設定されていません")
...
async def deep_research(question: str) -> str:
    ...
    try:
        # Open Deep ResearchのAgentによる調査の実行
